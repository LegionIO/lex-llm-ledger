        ...
        module Runners
          module Metrics
            ...
            def find_or_create(uuid:, attrs: {}, **)
              ...
              handle_exception(e, level: :warn, handled: true, operation: 'metrics.find_or_create_race')
              fetch(uuid: uuid)
            end
            ...
            def enrich(record:, updates:, **)
              ...
              Legion::Data::Models::LLM::MessageInferenceMetric.where(id: record[:id]).update(updates)
            ...
            def cache_record(result)
              ...
              cache_set("ledger:metric:id:#{result[:id]}", result, ttl: CACHE_TTL)
              cache_set("ledger:metric:uuid:#{result[:uuid]}", result, ttl: CACHE_TTL)
              cache_set("ledger:metric:req:#{result[:message_inference_request_id]}", result, ttl: CACHE_TTL)
            end
